...
def test_provider_failure_falls_back(monkeypatch) -> None:
    ...
    def ok_provider(chunk_id, redacted_code, signals, context):
        return {
    ...
    draft = router.refine_with_router("c1", "code", [], {})

    assert draft["provider"] == "openai"
    assert draft["type"] == "sql_injection"


def test_openai_provider_parses_chat_completion(monkeypatch) -> None:
    ...
    monkeypatch.setattr("app.services.analysis.llm_providers.openai_compatible.httpx.Client", FakeClient)

    draft = refine_openai("c1", "code", [], {})
